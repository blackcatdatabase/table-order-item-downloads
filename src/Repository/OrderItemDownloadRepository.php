...
use BlackCat\Database\Packages\OrderItemDownloads\Repository\OrderItemDownloadRepositoryInterface;
...
class OrderItemDownloadRepository implements OrderItemDownloadRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'tenant_id', 'order_id', 'book_id', 'asset_id' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\OrderItemDownloads\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'download_token_hash', 'token_key_version', 'key_version', 'max_uses', 'used', 'expires_at', 'last_used_at', 'ip_hash', 'ip_hash_key_version' ]);
          return count($rows);
        }
    ...
    public function getByTenantIdAndOrderIdAndBookIdAndAssetId(int $tenantId, int $orderId, int $bookId, int $assetId, bool $asDto = false): array|\BlackCat\Database\Packages\OrderItemDownloads\Dto\OrderItemDownloadDto|null {
        return $this->getByUnique([ 'tenant_id' => $tenantId, 'order_id' => $orderId, 'book_id' => $bookId, 'asset_id' => $assetId ], $asDto);
    }
    ...
    public function getIdByTenantIdAndOrderIdAndBookIdAndAssetId(int $tenantId, int $orderId, int $bookId, int $assetId) {
        $row = $this->getByTenantIdAndOrderIdAndBookIdAndAssetId($tenantId, $orderId, $bookId, $assetId, false);
