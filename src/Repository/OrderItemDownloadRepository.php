...
namespace BlackCat\Database\Packages\OrderItemDownloads\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\OrderItemDownloads\Definitions;
use BlackCat\Database\Packages\OrderItemDownloads\Criteria;
use BlackCat\Database\Packages\OrderItemDownloads\Dto\OrderItemDownloadDto as Dto;
use BlackCat\Database\Packages\OrderItemDownloads\Mapper\OrderItemDownloadDtoMapper as RowMapper;
...
final class OrderItemDownloadRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\OrderItemDownloads\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $keys = [ 'tenant_id', 'order_id', 'book_id', 'asset_id' ];
        if (!$keys) {
            $uqs = Definitions::uniqueKeys();
            $keys = (is_array($uqs) && isset($uqs[0]) && is_array($uqs[0]) && $uqs[0]) ? $uqs[0] : $this->pkColumns(Definitions::class);
        }

        $updCols = [ 'download_token_hash', 'token_key_version', 'key_version', 'max_uses', 'used', 'expires_at', 'last_used_at', 'ip_hash', 'ip_hash_key_version' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'tenant_id', 'order_id', 'book_id', 'asset_id' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\OrderItemDownloads\Definitions::table());
            return $bulk->upsertMany($rows, [ 'tenant_id', 'order_id', 'book_id', 'asset_id' ], [ 'download_token_hash', 'token_key_version', 'key_version', 'max_uses', 'used', 'expires_at', 'last_used_at', 'ip_hash', 'ip_hash_key_version' ]);
        }
    ...
    */
    ...
    public function getByTenantIdAndOrderIdAndBookIdAndAssetId(int $tenantId, int $orderId, int $bookId, int $assetId, bool $asDto = false): array|\BlackCat\Database\Packages\OrderItemDownloads\Dto\OrderItemDownloadDto|null {
        $row = $this->getByUnique([ 'tenant_id' => $tenantId, 'order_id' => $orderId, 'book_id' => $bookId, 'asset_id' => $assetId ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\OrderItemDownloads\Mapper\OrderItemDownloadDtoMapper::fromRow($row);
    }
    public function existsByTenantIdAndOrderIdAndBookIdAndAssetId(int $tenantId, int $orderId, int $bookId, int $assetId): bool {
        $where = 't.' . Ident::q($this->db, 'tenant_id') . ' = :uniq_tenant_id' . ' AND ' . 't.' . Ident::q($this->db, 'order_id') . ' = :uniq_order_id' . ' AND ' . 't.' . Ident::q($this->db, 'book_id') . ' = :uniq_book_id' . ' AND ' . 't.' . Ident::q($this->db, 'asset_id') . ' = :uniq_asset_id';
        return $this->exists($where, [ 'uniq_tenant_id' => $tenantId, 'uniq_order_id' => $orderId, 'uniq_book_id' => $bookId, 'uniq_asset_id' => $assetId ]);
    }
    /** @return int|string|null */
    public function getIdByTenantIdAndOrderIdAndBookIdAndAssetId(int $tenantId, int $orderId, int $bookId, int $assetId) {
        $row = $this->getByTenantIdAndOrderIdAndBookIdAndAssetId($tenantId, $orderId, $bookId, $assetId);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\OrderItemDownloads\Dto\OrderItemDownloadDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\OrderItemDownloads\Dto\OrderItemDownloadDto|null {
