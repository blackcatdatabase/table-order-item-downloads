...
final class OrderItemDownloadsModule implements ModuleInterface
{
    public function name(): string { return 'table-order_item_downloads'; }
    public function table(): string { return 'order_item_downloads'; }
    public function version(): string { return '1.0.0'; }

    /** @return string[] */
    public function dialects(): array { return [ 'mysql', 'postgres' ]; }
    /** @return string[] */
    public function dependencies(): array { return [ 'table-book_assets', 'table-books', 'table-orders', 'table-tenants' ]; }
...
SQL;
    ...
    public function status(Database $db, SqlDialect $d): array
    {
        ...
        $expectedIdx = [ 'idx_oid_download_token_hash', 'idx_oid_expires_at', 'idx_oid_tenant_expires_active', 'idx_oid_tenant_order', 'ux_oid_tenant_id', 'ux_oid_tenant_triplet' ];
        if ($d->isMysql()) {
    ...
    public function info(): array
    {
        return [
            ...
            'indexes'     => [ 'idx_oid_download_token_hash', 'idx_oid_expires_at', 'idx_oid_tenant_expires_active', 'idx_oid_tenant_order', 'ux_oid_tenant_id', 'ux_oid_tenant_triplet' ],
