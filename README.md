...
<div style='font-size:13px;opacity:0.92;'>Lineage heat: 4 outbound / 0 inbound · ✅ No engine drift detected · Index coverage: ready · PII: 5 · Changelog: fresh (18 d)</div>
</div>
...
- Pkg folder: [packages\order-item-downloads](https://github.com/blackcatacademy/blackcat-database/blob/main/packages\order-item-downloads)
...
graph LR
  ...
  order_item_downloads["order_item_downloads"]:::center
  order_item_downloads -->|FK| book_assets["book_assets"]:::link
  order_item_downloads -->|FK| books["books"]:::accent
  order_item_downloads -->|FK| orders["orders"]:::link
  order_item_downloads -->|FK| tenants["tenants"]:::accent
...
- Outbound (depends on): "book_assets", "books", "orders", "tenants"
...
- download_token_hash (token)
- id (key)
- ip_hash_key_version (key)
- key_version (key)
- token_key_version (token)

## Usage
- Install/upgrade schema: pwsh -NoLogo -NoProfile -File scripts/schema-tools/Migrate-DryRun.ps1 -Package order-item-downloads -Apply
